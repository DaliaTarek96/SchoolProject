...
export class TableComponent implements OnInit {
 ...
 save(){
  ...
  if(this.check==0){this.subjectService.setTable(this.table,this.Allsubject._id).subscribe(data=>this.Router.navigate(['/StudentAffaires/subject']))}
 }
 Resertable(){
        ...
        this.subjectService.getTeachers().subscribe(data=>{
          data.forEach(element => {//array to get  each Teachers  
            element.subjects.classesName.forEach(clas => {// array for get classes Teachers
              if(this.Allsubject.ClassNo == clas)
              {this.Teachers.push({_id:element._id,fullName:element.fullName,Class:clas,Subject:element.subjects.subjectName})} // to get teachers teach to this class
            });
          });
        })
      });
